...
def heapify(SourceGame, arr, N, i):
    ...
    if l < N and SourceGame.similarity_score(arr[l]) > SourceGame.similarityy_score(arr[largest]):
        largest = l
...
def Percolate(SourceGame, arr, N, ParentIndex):
    ...
    if((2*ParentIndex)+2 < N): 
        ...
        if SourceGame.similarity_score(arr[(2*ParentIndex)+1]) > SourceGame.similarity_score(arr[(2*ParentIndex)+2]): #.SimScore > arr[(2*ParentIndex)+2].SimScore:
